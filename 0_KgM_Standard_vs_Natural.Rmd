...
76829 Landau\
GERMANY\
Corresponding author mail address: schreiner-verena'\@'uni-landau.de, schreiner.verena'\@'rptu.de
...
$$ DSM = \left( \frac{\frac{AFDM_{t0}-AFDM_{t1}}{AFDM_{t0}} *100}{Tn} \right)$$
...
```{r}
leaf_weight_2$DSM <- (((leaf_weight_2$afdm_start - leaf_weight_2$afdm_end) /
                              leaf_weight_2$afdm_start) * 100)/
                   (leaf_weight_2$temp * as.numeric(leaf_weight_2$duration))

# remove one negative value (weighing error)
leaf_weight_2 <- leaf_weight_2[leaf_weight_2$DSM > 0, ]
```

calculate decomposed mass over whole period (not normalised for temperature and per day)
```{r}
leaf_weight_2$DSM_absolute <- (((leaf_weight_2$afdm_start - leaf_weight_2$afdm_end) /
                              leaf_weight_2$afdm_start) * 100)
```
...
```{r}
leaf_weight_2 <- leaf_weight_2[,c("siteID", "rep_number", "DSM")]
...
# calculate decomposed mass over whole period (not normalised for temperature and per day)
Tens_loss_abs <- (((mean(Cotton_2019_blank$Peak.tension) - Cotton_2019$Peak.tension) /
                             mean(Cotton_2019_blank$Peak.tension)) * 100)
...
```{r}
Decotabs_2019$DSM <- (((mean_Decot_trans - Decotabs_2019$afdm_end) /
                              mean_Decot_trans) * 100)/
                   (Decotabs_2019$temp * as.numeric(Decotabs_2019$duration))
```

calculate abolute decomposed substrate mass
```{r}
Decotabs_2019$DSM_abs <- (((mean_Decot_trans - Decotabs_2019$afdm_end) /
                              mean_Decot_trans) * 100)
```
...
```{r}
Decotabs_2019 <- Decotabs_2019[! is.na(Decotabs_2019$DSM) & Decotabs_2019$DSM > 0, ]
...
names(Decom_2019)[3] <- c("DSM_leaves")
...
names(Decom_2019)[5] <- c("DSM_Decotabs")
...
Decom_2019 <- Decom_2019[!is.na(Decom_2019$DSM_leaves) | !is.na(Decom_2019$Tens_loss) |
                           !is.na(Decom_2019$DSM_Decotabs), ]
...
```{r}
Macroinv_leaves$DSM <- (((Macroinv_leaves$afdm_start - Macroinv_leaves$afdm_end) /
...
nrow(Macroinv_leaves[Macroinv_leaves$DSM < 0, ]) # all values positive
```

calculate absolute decomposed substrate mass
```{r}
Macroinv_leaves$DSM_abs <- (((Macroinv_leaves$afdm_start - Macroinv_leaves$afdm_end) /
                              Macroinv_leaves$afdm_start) * 100)
```

only keep important columns
```{r}
Macroinv_leaves <- Macroinv_leaves[,c("siteID", "rep_number", "DSM")]
...
```{r}
Decotabs_coarse_2019$DSM <- (((mean_Decot_coarse_trans - Decotabs_coarse_2019$afdm_end) /
...
```{r}
Decotabs_coarse_2019 <- Decotabs_coarse_2019[! is.na(Decotabs_coarse_2019$DSM) & !Decotabs_coarse_2019$DSM < 0, ]
...
calculate absolute decomposed substrate mass 
```{r}
Decotabs_coarse_2019$DSM_abs <- (((mean_Decot_coarse_trans - Decotabs_coarse_2019$afdm_end) /
                              mean_Decot_coarse_trans) * 100)
```
...
```{r}
hist(Decom_2019$DSM_leaves)  # data not skewed

hist(Decom_2019$DSM_Decotabs) # right skewed, wide range of data
...
```{r}
# Decotabs
skewness(Decom_2019$DSM_Decotabs, na.rm = TRUE) # positively skewed, below |2|, but since over more than one magnitude transform at least for direct comparison
Decom_2019$DSM_Decotabs_tr <- log10(Decom_2019$DSM_Decotabs)
hist(Decom_2019$DSM_Decotabs_tr) # after transformation skew almost gone, use transformed data in direct comparision
skewness(Decom_2019$DSM_Decotabs_tr, na.rm = TRUE)
# 
# Cotton strips
...
```{r}
plot(Decom_2019$DSM_leaves, Decom_2019$DSM_Decotabs, pch = 19, ylab = "Decotabs", xlab = "leaves")
cor.test(Decom_2019$DSM_leaves, Decom_2019$DSM_Decotabs)$p.value
round(as.numeric(cor.test(Decom_2019$DSM_leaves, Decom_2019$DSM_Decotabs)$estimate)^2,3) # r2 = 0.01
```

plot transformed data

```{r}
plot(Decom_2019$DSM_leaves, Decom_2019$DSM_Decotabs_tr, pch = 19, ylab = "Decotabs transformed", xlab = "leaves")
cor.test(Decom_2019$DSM_leaves, Decom_2019$DSM_Decotabs_tr)$p.value
round(as.numeric(cor.test(Decom_2019$DSM_leaves, Decom_2019$DSM_Decotabs_tr)$estimate)^2,3) # r2 < 0.01
```
...
```{r}
plot(Decom_2019$DSM_leaves, Decom_2019$Tens_loss, pch = 19, ylab = "Cottonstrips", xlab = "leaves")
cor.test(Decom_2019$DSM_leaves, Decom_2019$Tens_loss)$p.value
round(as.numeric(cor.test(Decom_2019$DSM_leaves, Decom_2019$Tens_loss)$estimate)^2,3) # r2 = 0.026
```
...
```{r}
plot(Decom_2019$DSM_Decotabs, Decom_2019$Tens_loss, pch = 19, xlab = "Decotabs", ylab = "Cottonstrips")
cor.test(Decom_2019$DSM_Decotabs, Decom_2019$Tens_loss)$p.value 
round(as.numeric(cor.test(Decom_2019$DSM_Decotabs, Decom_2019$Tens_loss)$estimate)^2,3) # r2 = 0.126
```
...
```{r}
plot(Decom_2019$DSM_Decotabs_tr, Decom_2019$Tens_loss, pch = 19, xlab = "Decotabs", ylab = "Cottonstrips")
cor.test(Decom_2019$DSM_Decotabs_tr, Decom_2019$Tens_loss)$p.value
round(as.numeric(cor.test(Decom_2019$DSM_Decotabs_tr, Decom_2019$Tens_loss)$estimate)^2,3) # r2 = 0.206
```
...
p1 <- ggplot(data = Decom_2019, aes(x = DSM_leaves, y = DSM_Decotabs))+ 
  scale_y_log10() + 
  geom_point(size = 2) +
  ggtitle("") +
  ...
  labs(subtitle = "A") +
  annotate("text", x = 0.04, y=0.012, label = paste("~italic(r)^2 <", 0.01), parse=TRUE, size = 3.5)+ 
  annotate("text", x = 0.04, y=0.008, label = "italic(p) == 0.898", parse=TRUE,  size = 3.5)+
  theme_classic()+
...
p2 <- ggplot(data = Decom_2019, aes(x = DSM_leaves, y = Tens_loss))+ 
  geom_point(size = 2) +
  ggtitle("") +
  ylab("Tension loss  [%] per dday")+
  xlab("Decomposed leaf mass [%] per dday") +
  labs(subtitle = "B") +
  annotate("text", x = 0.04, y=0.05, label = paste("~italic(r)^2 ==", 0.03), parse=TRUE, size = 3.5)+ 
    annotate("text", x = 0.04, y=0.02, label = "italic(p) == 0.053", parse=TRUE, size = 3.5)+
  theme_classic()+
  theme(axis.text.x = element_text(colour="black", size = 12), 
        axis.text.y = element_text(colour="black", size = 12), 
        plot.subtitle = element_text(colour="black", size = 14),
...
# decotabs transformed
p3 <- ggplot(data = Decom_2019, aes(x = DSM_Decotabs, y = Tens_loss))+ 
  scale_x_log10() + 
  geom_point(size = 2) +
  ggtitle("") +
  ylab("Tension loss  [%] per dday")+
  ...
  labs(subtitle = "C") +
  annotate("text", x = 0.25, y=0.05, label = paste("~italic(r)^2 ==", 0.21), parse=TRUE, size = 3.5)+ 
  annotate("text", x = 0.25, y=0.02, label = "italic(p) < 0.001", parse=TRUE, size = 3.5)+
  theme_classic()+
  theme(axis.text.x = element_text(colour="black", size = 12), 
        axis.text.y = element_text(colour="black", size = 12), 
        plot.subtitle = element_text(colour="black", size = 14),
...
```{r, warning=FALSE}
p1_n_t <- ggplot(data = Decom_2019, aes(x = DSM_leaves, y = DSM_Decotabs))+ 
  geom_point(size = 2) +
  ggtitle("") +
  ylab("Decomposed decotabs mass [%] per dday")+
  xlab("Decomposed leaf mass [%] per dday") +
  labs(subtitle = "A") +
  annotate("text", x = 0.04, y=0.31, label = paste("~italic(r)^2 ==", 0.01), parse=TRUE, size = 3.5)+ 
    annotate("text", x = 0.04, y=0.29, label = "italic(p) == 0.206", parse=TRUE, size = 3.5)+
  theme_classic()+
  theme(axis.text.x = element_text(colour="black", size = 12), 
        axis.text.y = element_text(colour="black", size = 12), 
        plot.subtitle = element_text(colour="black", size = 14),
...
# Decotabs vs. cotton strips
# decotabs transformed
p3_n_t <- ggplot(data = Decom_2019, aes(x = DSM_Decotabs, y = Tens_loss))+ 
  geom_point(size = 2) +
  ggtitle("") +
  ylab("Tension loss  [%] per dday")+
  xlab("Decomposed decotabs mass [%] per dday") +
  labs(subtitle = "C") +
  annotate("text", x = 0.25, y=0.05, label = paste("~italic(r)^2 ==", 0.13), parse=TRUE, size = 3.5)+ 
  annotate("text", x = 0.25, y=0.02, label = "italic(p) < 0.001", parse=TRUE, size = 3.5)+
  theme_classic()+
  theme(axis.text.x = element_text(colour="black", size = 12), 
        axis.text.y = element_text(colour="black", size = 12), 
...
```{r}
# absolute
mean(with(Decom_2019, tapply(DSM_leaves, siteID, var)), na.rm = TRUE) # 0.000682934
# relative [%]
mean(with(Decom_2019, tapply(DSM_leaves, siteID, var))/ with(Decom_2019, tapply(DSM_leaves, siteID, mean)), na.rm = TRUE)*100 # 0.46 %
```
...
```{r}
# absolute
mean(with(Decom_2019, tapply(DSM_Decotabs, siteID, var)), na.rm = TRUE) # 0.001533229
# relative [%]
mean(with(Decom_2019, tapply(DSM_Decotabs, siteID, var))/ with(Decom_2019, tapply(DSM_Decotabs, siteID, mean)), na.rm = TRUE)*100 # 1.51 %
```
...
```{r}
hist(Decom_2019_all$DSM_leaves_coarse)  # data very slightly left skewed

hist(Decom_2019_all$DSM_Decotabs_coarse) # data not skewed
# check skewness
skewness(Decom_2019_all$DSM_leaves_coarse, na.rm = TRUE) 
skewness(Decom_2019_all$DSM_Decotabs_coarse, na.rm = TRUE)
```
...
```{r}
plot(Decom_2019_all$DSM_leaves_coarse, Decom_2019_all$DSM_Decotabs_coarse, pch = 19, ylab = "Decotabs", xlab = "leaves")
cor.test(Decom_2019_all$DSM_leaves_coarse, Decom_2019_all$DSM_Decotabs_coarse)$p.value
round(as.numeric(cor.test(Decom_2019_all$DSM_leaves_coarse, Decom_2019_all$DSM_Decotabs_coarse)$estimate)^2,3) # r2 = 0.038
```

comparison over 66 observations from 31 sites

```{r}
data_macro <- Decom_2019_all[!is.na(Decom_2019_all$DSM_leaves_coarse), ]
data_macro <- data_macro[!is.na(data_macro$DSM_Decotabs_coarse), ]
```
...
# Leaves vs. Decotabs
ggplot(data = Decom_2019_all, aes(x = DSM_leaves_coarse, y = DSM_Decotabs_coarse))+ 
  geom_point(size = 2) +
  ggtitle("") +
  ylab("Decomposed decotabs mass [%] per dday")+
  xlab("Decomposed leaf mass [%] per dday") +
  xlim(0.13, 0.9)+
  annotate("text", x = 0.85, y=0.25, label = paste("~italic(r)^2 ==",  0.04), parse=TRUE, size = 3.5)+ 
  annotate("text", x = 0.85, y=0.23, label = "italic(p) == 0.12", parse=TRUE, size = 3.5)+
  theme_classic()+
  theme(axis.text.x = element_text(colour="black", size = 12), 
        axis.text.y = element_text(colour="black", size = 12), 
        plot.subtitle = element_text(colour="black", size = 14),
...
```{r}
# absolute
mean(with(Decom_2019_all, tapply(DSM_leaves_coarse, siteID, var)), na.rm = TRUE) # 0.007914277

# relative [%]
mean(with(Decom_2019_all, tapply(DSM_leaves_coarse, siteID, var))/ with(Decom_2019_all, tapply(DSM_leaves_coarse, siteID, mean)), na.rm = TRUE)*100 # 1.73%
```
...
```{r}
# absolute
mean(with(Decom_2019_all, tapply(DSM_Decotabs_coarse, siteID, var)), na.rm = TRUE) # 0.0008586815
# relative [%]
mean(with(Decom_2019_all, tapply(DSM_Decotabs_coarse, siteID, var))/ with(Decom_2019_all, tapply(DSM_Decotabs_coarse, siteID, mean)), na.rm = TRUE)*100 # 0.74 %
```
...
model_t1 <- lm(DSM_leaves ~ streamType + ratio_agri + fine_sub + area_tr + fine_sub + speartaxa + perc_shred + CPOM_tr + EC_t1 + O2_t1 + NO2_t1 + PO4_t1_tr + NH4_t1_tr + TP_t1_tr + TNb_t1_tr + NP_ratio_t1_tr + As_t1_tr + Zn_t1_tr + Hg_t1_tr + Cu_t1 + pH_t1 + flow_t1 + sumTU_fu_date_t1 + sumTU_iv_date_t1, data = Decom_2019_t1)
car::vif(model_t1)
```
...
```{r}
model_t1_2 <- lm(DSM_leaves ~ streamType + ratio_agri + fine_sub + area_tr + fine_sub + speartaxa + perc_shred + CPOM_tr + EC_t1 + O2_t1 + NO2_t1 + PO4_t1_tr + NH4_t1_tr + TP_t1_tr + TNb_t1_tr + As_t1_tr + Zn_t1_tr + Hg_t1_tr + Cu_t1 + pH_t1 + flow_t1 + sumTU_fu_date_t1 + sumTU_iv_date_t1, data = Decom_2019_t1)
car::vif(model_t1_2)
```
...
model_t2 <- lm(DSM_leaves ~ streamType + ratio_agri + fine_sub + area_tr + fine_sub + speartaxa + perc_shred + CPOM_tr + EC_t2 + O2_t2 + NO2_t2_tr + PO4_t2_tr + NH4_t2_tr + TP_t2_tr + TNb_t2_tr + NP_ratio_t2_tr + As_t2 + Zn_t2_tr + Hg_t2_tr + Cu_t2 + pH_t2 + flow_t2 + sumTU_fu_date_t2 + sumTU_iv_date_t2, data = Decom_2019_t2)
car::vif(model_t2)
```
...
```{r}
model_t2_2 <- lm(DSM_leaves ~ streamType + ratio_agri + fine_sub + area_tr + fine_sub + speartaxa + perc_shred + CPOM_tr + EC_t2 + O2_t2 + NO2_t2_tr + PO4_t2_tr + NH4_t2_tr + TP_t2_tr + TNb_t2_tr + As_t2 + Zn_t2_tr + Hg_t2_tr + Cu_t2 + pH_t2 + flow_t2 + sumTU_fu_date_t2 + sumTU_iv_date_t2, data = Decom_2019_t2)
car::vif(model_t2_2)
```
now VIF is fine (all clearly below 10, even below 5)
...
```{r}
Decom_2019_leaves <- Decom_2019_t1[!is.na(Decom_2019_t1$DSM_leaves), ]
...
res_leaves <- as.matrix(select(Decom_2019_leaves, DSM_leaves))  # --> response variable
data_en_leaves <- scale(as.matrix(data_en_leaves)) # --> explanatory variables
```

remove the variables EC and O2, due to missing values (NA).
See in previous version, results where these variables were included (before change of package), results are similar
```{r}
data_en_leaves <- data_en_leaves[, -c(4,5)]
```
...
See in previous version, results where these variables were included (before change of package), results are similar
```{r}
data_en_cotton <- data_en_cotton[, -c(4,5)]
```
...
```{r}
Decom_2019_dec <- Decom_2019_t1[!is.na(Decom_2019_t1$DSM_Decotabs), ]
# 173 observations
```
...
```{r}
#  explanatory variables need to be Decom_2019_cotton
res_dec <- as.matrix(select(Decom_2019_dec, DSM_Decotabs))  # --> response variable -> y
...
```{r}
Decom_2019_leaves_coarse <- Decom_2019_t2[!is.na(Decom_2019_t2$DSM_leaves_coarse), ]
...
```{r, trace= FALSE, echo=FALSE, message=FALSE}
#  explanatory variables need to be Decom_2019_cotton
res_leaves_coarse <- as.matrix(select(Decom_2019_leaves_coarse, DSM_leaves_coarse))  # --> response variable -> y
data_en_leaves_coarse <- scale(as.matrix(data_en_leaves_coarse)) # --> explanatory variables -> x
```

remove the variables EC and O2, due to missing values (NA).
Remove three entries of missing nutrient values 
See in previous version, results where these variables were included (before change of package), results are similar
```{r}
data_en_leaves_coarse <- data_en_leaves_coarse[, -c(6,7)]
data_en_leaves_coarse <- data_en_leaves_coarse[-c(32,33,34), ]
```
...
```{r}
set.seed(123) # set seed to enable better reproducibility (still not 100% reproducibility)
(stab.lasso_leaves_coarse <- stabsel(x = data_en_leaves_coarse, y = res_leaves_coarse[-c(32,33,34)],
                             fitfun = glmnet.lasso, cutoff = 0.7,
                             PFER = 1))
```
...
```{r}
Decom_2019_dec_coarse <- Decom_2019_t1[!is.na(Decom_2019_t1$DSM_Decotabs_coarse), ]
# 80 observations
```
...
```{r, trace= FALSE, echo=FALSE, message=FALSE}
#  explanatory variables need to be Decom_2019_cotton
res_dec_coarse <- as.matrix(select(Decom_2019_dec_coarse, DSM_Decotabs_coarse))  # --> response variable -> y
...
See in previous version, results where these variables were included (before change of package), results are similar
```{r}
data_en_dec_coarse <- data_en_dec_coarse[, -c(6,7)]
```
...
```{r}
Decom_all_leaves <- Decom_all[!is.na(Decom_all$DSM_leaves), ]
m1 <- lm(DSM_leaves ~ streamType + log10(area) + ratio_agri + fine_sub + O2_t1 + pH_t1 + EC_t1 + flow_t1 + sumTU_fu_date_t1 + log10(PO4_t1) + NO2_t1 + log10(NH4_t1) + log10(TP_t1) + log10(TNb_t1)  + log10(As_t1) + Cu_t1 + log10(Hg_t1) + log10(Zn_t1) , data = Decom_all_leaves)
...
```{r}
Decom_all_deco <- Decom_all[!is.na(Decom_all$DSM_Decotabs), ]
m3 <- lm(DSM_Decotabs ~ streamType + log10(area) + ratio_agri + fine_sub + O2_t1 + pH_t1 + EC_t1 + flow_t1 + sumTU_fu_date_t1 + log10(PO4_t1) + NO2_t1 + log10(NH4_t1) + log10(TP_t1) + log10(TNb_t1)  + log10(As_t1) + Cu_t1 + log10(Hg_t1) + log10(Zn_t1), data = Decom_all_deco)
...
```{r}
Decom_all_leaves_coarse <- Decom_all[!is.na(Decom_all$DSM_leaves_coarse), ]

m4 <- lm(DSM_leaves_coarse ~ streamType + log10(area) + ratio_agri + fine_sub + O2_t2 + pH_t2 + EC_t2 + flow_t2 + sumTU_fu_date_t2 + log10(PO4_t2) + log10(NO2_t2) + log10(NH4_t2) + log10(TP_t2) + log10(TNb_t2)  + As_t2 + Cu_t2 + log10(Hg_t2) + log10(Zn_t2) + sumTU_iv_date_t2 + CPOM + speartaxa + perc_shred, data = Decom_all_leaves_coarse)
...
```{r}
Decom_all_deco_coarse <- Decom_all[!is.na(Decom_all$DSM_Decotabs_coarse), ]

m5 <- lm(DSM_Decotabs_coarse ~ streamType + log10(area) + ratio_agri + fine_sub + O2_t1 + pH_t1 + EC_t1 + flow_t1 + sumTU_fu_date_t1 + log10(PO4_t1) + NO2_t1 + log10(NH4_t1) + log10(TP_t1) + log10(TNb_t1)  + log10(As_t1) + Cu_t1 + log10(Hg_t1) + log10(Zn_t1) + CPOM + sumTU_iv_date_t1 + speartaxa + perc_shred, data = Decom_all_deco_coarse)
